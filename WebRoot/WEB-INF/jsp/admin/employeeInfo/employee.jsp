		...
		$(function(){
			...
			$("#addEmployee").click(function(){
				...
				$("#divDialog").dialog({
					...
					onClose : function(){ // 监听是否关闭当前窗口
						window.location.href = "${path}/admin/employeeInfo/selectEmployee.jspx?pageModel.pageIndex=${pageModel.pageIndex}&employee.name=${employee.name}&employee.phone=${employee.phone}&employee.dept.id=${employee.dept.id}";
					}
				});
				$("#iframe").prop("src", "${path}/admin/employeeInfo/showAddEmployee.jspx").show();
			});
			...
			$("#updateEmployee").click(function(){
				...
				if (boxs.length == 0){
					...
					$("#iframe").prop("src", "${path}/admin/employeeInfo/showUpdateEmployee.jspx?employee.employeeId=" + boxs.val()).show();
				}else{
					alert("修改时，只能选择一个用户！");
				}
			});
			...
			$("#deleteEmployee").click(function(){
				...
				if (boxs.size() == 0){
					...
					if (confirm("您确定要删除吗？")){
						...
						window.location.href = "${path}/admin/employeeInfo/deleteEmployee.jspx?pageModel.pageIndex=${pageModel.pageIndex}&employee.name=${employee.name}&employee.phone=${employee.phone}&employee.dept.id=${employee.dept.id}&employeeIds=" + employeeIds.join(",");
					}
				}
			});
...
<body>
	<!-- 工具按钮区 -->
	<s:form action="/admin/employeeInfo/selectEmployee.jspx" method="post" theme="simple">
		<table>
			<tr>
				<td><input type="button" value="添加" id="addEmployee"/></td>
				<td><input type="button" value="修改" id="updateEmployee"/></td>
				<td><input type="button" value="删除" id="deleteEmployee"/></td>
				<td>姓名：<s:textfield name="employee.name" size="12" id="nameText"/></td>
				<td>手机号码：<s:textfield name="employee.phone" size="12"/></td>
				<td>部门：<select name="employee.dept.id" id="deptSelect">
		...
		<tbody style="background-color: #FFFFFF;">
			<s:iterator value="employees" status="stat">
				<tr id="tr_${stat.index}" class="listTr">
					<td><input type="checkbox" id="box_${stat.index}" value="${employeeId}"/>${stat.count}</td>
					...
					<td>
						<s:if test="status == 0">在职</s:if>
						<s:elseif test="status == 1"><font color="red">离职</font></s:elseif>
					</td>
	...
	<it:pager pageIndex="${pageModel.pageIndex}" 
			  ...
			  submitUrl="${path}/admin/employeeInfo/selectEmployee.jspx?pageModel.pageIndex={0}&employee.name=${employee.name}&employee.phone=${employee.phone}&user.dept.id=${employee.dept.id}"/>
