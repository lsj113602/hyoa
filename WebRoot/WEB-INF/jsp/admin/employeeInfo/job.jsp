		...
		$(function(){
			...
			$("#addJob").click(function(){
				...
				$("#divDialog").dialog({
					...
					onClose : function(){ // 监听是否关闭当前窗口
						window.location.href = "${path}/admin/employeeInfo/selectJob.jspx?pageModel.pageIndex=${pageModel.pageIndex}";
					}
				});
				$("#iframe").prop("src", "${path}/admin/employeeInfo/showAddJob.jspx").show();
			});
			...
			$("#updateJob").click(function(){
				...
				if (boxs.length == 0){
					...
					$("#iframe").prop("src", "${path}/admin/employeeInfo/showUpdateJob.jspx?job.code=" + boxs.val()).show();
				}else{
					alert("修改时，只能选择一个职位！");
				}
			});
			...
			$("#deleteJob").click(function(){
				...
				if (boxs.size() == 0){
					...
					if (confirm("您确定要删除吗？")){
						...
						window.location.href = "${path}/admin/employeeInfo/deleteJob.jspx?pageModel.pageIndex=${pageModel.pageIndex}&codes=" + codes.join(",");
					}
				}
			});
...
<body>
	<table>
		<tr>
			...
			<td><input type="button" value="删除" id="deleteJob"/>
	...
	<table width="100%" class="listTable" cellpadding="8" cellspacing="1">
		...
		<tbody style="background-color: #FFFFFF;">
			<s:iterator value="jobs" status="stat">
	...
	<it:pager pageIndex="${pageModel.pageIndex}" 
			  ...
			  submitUrl="${path}/admin/employeeInfo/selectJob.jspx?pageModel.pageIndex={0}"/>
